...
export default function StudentUpsertForm({handleSubmit,values}) {
  ...
  useEffect(() => {
    ParentApi.index(['id', 'first_name', 'last_name']).then(({data}) => setParents(data.data) )
  }, []);
              ...
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                ...
                <SelectContent>
                  {parents.map((parent, key) =>
                    <SelectItem key={key} value={parent.id.toString()}>{parent.first_name} {parent.last_name}</SelectItem>)
                  }
                </SelectContent>
              </Select>
              <FormMessage/>
